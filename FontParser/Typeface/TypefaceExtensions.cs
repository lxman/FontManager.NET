...
namespace FontParser.Typeface
{
    public static class TypefaceExtensions
    {
        ...
        internal static TranslatedOs2FontStyle TranslateOs2FontStyle(Os2Table os2Table)
        {
            ...
            {
                result |= TranslatedOs2FontStyle.ITALIC;
            }

            if (((fsSelection >> 5) & 0x1) != 0)
            {
                result |= TranslatedOs2FontStyle.BOLD;
            }

            if (((fsSelection >> 6) & 0x1) != 0)
            {
                result |= TranslatedOs2FontStyle.REGULAR;
            }
            if (((fsSelection >> 9) & 0x1) != 0)
            {
                result |= TranslatedOs2FontStyle.OBLIQUE;
            }
        ...
        private static int Calculate_BTBD_Windows(Typeface typeface)
        {
            ...
            int external_leading = Math.Max(0, hhea.LineGap - ((usWinAscent + usWinDescent) - (hhea.Ascent - hhea.Descent)));
        ...
        public static bool IsTrimmed(this Typeface typeface) => typeface._typefaceTrimMode == TrimMode.EssentialLayoutInfo;
        ...
        {
            //use default opentype font reader
