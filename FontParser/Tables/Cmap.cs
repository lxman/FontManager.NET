...
using FontParser.Tables.CharacterMap;
using FontParser.Tables.CharacterMap.CharMapFormats;
...
namespace FontParser.Tables
{
    ...
    public class Cmap : TableEntry
    {
        ...
        public ushort GetGlyphIndex(int codepoint, int nextCodepoint, out bool skipNextCodepoint)
        {
            ...
            {
                foreach (CharacterMap.CharacterMap cmap in _charMaps)
                {
                    if (found == 0)
                    {
            ...
            //  -> otherwise, return 0
            if (nextCodepoint <= 0 || _charMap14List == null) return found;
            {
        ...
        protected override void ReadContentFrom(BinaryReader input)
        {
            ...
            _charMaps = new CharacterMap.CharacterMap[tableCount];
            for (var i = 0; i < tableCount; i++)
            {
                input.BaseStream.Seek(beginAt + offsets[i], SeekOrigin.Begin);
                CharacterMap.CharacterMap cmap = ReadCharacterMap(input);
        ...
        private static CharacterMap.CharacterMap ReadFormat_0(BinaryReader input)
        {
        ...
        private static CharacterMap.CharacterMap ReadFormat_2(BinaryReader input)
        {
        ...
        private static CharMapFormat4 ReadFormat_4(BinaryReader input)
        {
            ...
            ushort[] endCode = input.ReadUInt16Array(segCount);//Ending character code for each segment, last = 0xFFFF.
            ...
            ushort[] startCode = input.ReadUInt16Array(segCount); //Starting character code for each segment
            ushort[] idDelta = input.ReadUInt16Array(segCount); //Delta for all character codes in segment
            ushort[] idRangeOffset = input.ReadUInt16Array(segCount); //Offset in bytes to glyph indexArray, or 0
                                                                      //------------------------------------------------------------------------------------
            long remainingLen = tableStartEndAt - input.BaseStream.Position;
            var recordNum2 = (int)(remainingLen / 2);
            ushort[] glyphIdArray = input.ReadUInt16Array(recordNum2);//Glyph index array
        ...
        private static CharMapFormat6 ReadFormat_6(BinaryReader input)
        {
            ...
            ushort[] glyphIdArray = input.ReadUInt16Array(entryCount);
            return new CharMapFormat6(firstCode, glyphIdArray);
        }

        private static CharacterMap.CharacterMap ReadFormat_12(BinaryReader input)
        {
        ...
        private static CharacterMap.CharacterMap ReadCharacterMap(BinaryReader input)
        {
        ...
        {
            foreach (CharacterMap.CharacterMap m in _charMaps)
            {
                m.CollectUnicodeChars(unicodes);
            }
        }
