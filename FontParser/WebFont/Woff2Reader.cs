...
namespace FontParser.WebFont
{
    ...
    internal class TransformedGlyf : UnreadTableEntry
    {
        ...
        {
            if (!(expectedResult is Glyf glyfTable)) throw new NotSupportedException();
        ...
        private static void ReconstructGlyfTable(BinaryReader reader, Woff2TableDirectory woff2TableDir, Glyf glyfTable)
        {
            ...
            //--------------------------------------------------------------------------------------------
#if DEBUG
            if (expectedCompositeStreamStartAt != reader.BaseStream.Position)
            {
                //***
...
#if DEBUG
            if (expectedBboxStreamStartAt != reader.BaseStream.Position)
            {
            }
#endif
            ...
            //instruction stream
#if DEBUG
            if (reader.BaseStream.Position < expectedInstructionStreamStartAt)
            {
            }
...
#if DEBUG
            if (reader.BaseStream.Position != expectedEndAt)
            {
            }
#endif
        ...
        private static Glyph BuildSimpleGlyphStructure(BinaryReader glyphStreamReader,
            ...
            byte[] flagStream, ref int flagStreamIndex)
        {
            ...
            {
                ushort numPoint = pntPerContours[pntContourIndex++];//increment pntContourIndex AFTER
                pointCount += numPoint;
        ...
        private static bool CompositeHasInstructions(BinaryReader reader, ushort compositeGlyphIndex)
        {
            ...
            //but this does not create actual composite glyph.
        ...
        private static Glyph ReadCompositeGlyph(Glyph[] createdGlyphs, BinaryReader reader, ushort compositeGlyphIndex, Glyph emptyGlyph)
        {
    ...
    internal class Woff2Reader
    {
        ...
        private static void dbugVerifyKnownTables()
        {
            ...
            {
                if (!uniqueNames.TryAdd(name, true))
                {
                    throw new Exception();
                }
            }
        ...
        internal bool Read(Typeface.Typeface typeface, BinaryReader reader, RestoreTicket ticket)
        {
            _header = ReadHeader(reader);
