...
namespace FontParser.TrueTypeInterpreter
{
    ...
    {
        private Typeface.Typeface _currentTypeFace;
        ...
        public void SetTypeFace(Typeface.Typeface typeface)
        {
            //still preserve this for compat with others,
            //wait for other libs...

            _currentTypeFace = typeface;
            MaxProfile maximumProfile = _currentTypeFace.MaxProfile;
    ...
    internal class SharpFontInterpreter
    {
        ...
        private void Execute(InstructionStream stream, bool inFunction, bool allowFunctionDefs)
        {
            ...
            while (!stream.Done)
            {
                ...
                switch (opcode)
                {
                    ...
                    case OpCode.SCANCTRL: /* instruction unsupported */ _stack.Pop(); break;
                    case OpCode.SCANTYPE: /* instruction unsupported */ _stack.Pop(); break;
                    case OpCode.SANGW: /* instruction unsupported */ _stack.Pop(); break;
        ...
        //TODO: review here again
        private static unsafe float* GetPoint(byte* data, int index) => (float*)(data + sizeof(GlyphPointF) * index);
        ...
        private class ExecutionStack
        {
            ...
            public void Swap()
            {
                ...
                (_s[_count - 1], _s[_count - 2]) = (_s[_count - 2], _s[_count - 1]);
            }
